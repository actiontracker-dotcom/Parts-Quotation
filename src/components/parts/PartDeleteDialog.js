...
export default function PartDeleteDialog({ part, onConfirm, onCancel, loading }) {
  if (!part) return null;
      ...
      <div className="relative w-full max-w-md rounded-xl bg-white shadow-xl">
        <div className="flex items-start justify-between p-5 pb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-ink-900">Delete Part</h2>
        ...
        <div className="px-5 py-3 border-t border-ink-50">
          <p className="text-sm text-ink-700">
            ...
            <span className="font-semibold text-ink-900">{part.partNo}</span>?
          </p>
          {part.description && (
            <p className="text-sm text-ink-500 mt-1">{part.description}</p>
          )}
        </div>
